...
import {
  BookOpen, CheckCircle, ChevronDown, ChevronUp, ClipboardCheck, Eye, EyeOff,
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="pt-5 border-t border-stone-200 dark:border-slate-700 flex flex-wrap items-center gap-4">
        {!finished ? <button onClick={finish} disabled={Object.keys(answers).length < questions.length} className="px-7 py-3 rounded-xl bg-slate-900 dark:bg-blue-600 disabled:bg-stone-300 dark:disabled:bg-slate-700 text-white font-bold">{label}</button> : (
          <><div className="font-extrabold text-lg">Score: {score}/{questions.length}</div><button onClick={() => { setFinished(false); setAnswers({}); }} className="px-5 py-3 rounded-xl border border-stone-300 dark:border-slate-600 font-bold"><RefreshCw className="w-4 h-4 inline mr-2"/>Try again</button></>
        )}
...
function Flashcards({ cards, onComplete, saved }) {
  const categories = ['All', ...new Set(cards.map(card => card.category))];
  const [category, setCategory] = useState('All');
  const [flipped, setFlipped] = useState({});
  const visible = category === 'All' ? cards : cards.filter(card => card.category === category);
  return (
    <div className="space-y-6">
      <div className="rounded-3xl p-7 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
        <h2 className="text-2xl font-extrabold font-serif mb-2">English-only stative verb flashcards</h2>
        <p>Tap a card to turn the verb into a natural example sentence. No translations — connect the verb directly with its English context.</p>
      </div>
      <div className="flex flex-wrap gap-2">{categories.map(item => <button key={item} onClick={() => { setCategory(item); setFlipped({}); }} className={`px-4 py-2 rounded-full text-sm font-bold border ${category === item ? 'bg-slate-900 dark:bg-blue-600 text-white border-transparent' : 'bg-white dark:bg-slate-800 border-stone-300 dark:border-slate-600'}`}>{item}</button>)}</div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {visible.map((card, i) => {
          const key = `${category}-${i}-${card.verb}`;
          const isFlipped = !!flipped[key];
          return <button key={key} aria-label={`Flashcard: ${card.verb}`} aria-pressed={isFlipped} onClick={() => setFlipped(current => ({ ...current, [key]: !current[key] }))} className={`min-h-44 rounded-3xl p-6 border text-left transition-all ${isFlipped ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white dark:bg-slate-800 border-stone-200 dark:border-slate-700 hover:-translate-y-1 hover:shadow-lg'}`}>
            <div className={`text-xs font-black uppercase tracking-widest mb-5 ${isFlipped ? 'text-blue-100' : 'text-blue-600 dark:text-blue-300'}`}>{card.category}</div>
            <div className="text-2xl font-extrabold font-serif">{isFlipped ? card.example : card.verb}</div>
            <div className={`mt-5 text-xs font-bold ${isFlipped ? 'text-blue-100' : 'text-stone-400'}`}>{isFlipped ? 'Tap to see the verb' : 'Tap for an example'}</div>
          </button>;
        })}
      </div>
      <button onClick={() => onComplete('flashcards', 1, 1, {}, [])} className="px-7 py-3 rounded-xl bg-slate-900 dark:bg-blue-600 text-white font-bold">Mark flashcards complete</button>
      {saved && <span className="ml-4 text-sm font-bold text-emerald-600 dark:text-emerald-400">Saved in this browser</span>}
    </div>
  );
}

function ListeningTask({ task, onComplete, saved }) {
  const [plays, setPlays] = useState(0);
  const [showScript, setShowScript] = useState(false);
  const play = () => {
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      const speech = new SpeechSynthesisUtterance(task.script);
      speech.lang = 'en-GB';
      speech.rate = 0.88;
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-3xl p-7 bg-violet-50 dark:bg-violet-900/15 border border-violet-200 dark:border-violet-800">
        <Headphones className="w-8 h-8 text-violet-600 mb-3"/><h2 className="text-3xl font-extrabold font-serif mb-2">{task.title}</h2><p className="font-bold mb-2">Original practice audio · {task.level}</p><p className="mb-5">{task.instructions}</p>
        <div className="flex flex-wrap gap-3"><button onClick={play} className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-violet-600 text-white font-bold"><Play className="w-4 h-4"/>Play audio</button><button disabled={plays < 2} onClick={() => setShowScript(value => !value)} className="px-5 py-3 rounded-xl border border-violet-300 dark:border-violet-700 font-bold disabled:opacity-40">{showScript ? 'Hide' : 'Show'} transcript</button></div>
        <p className="mt-3 text-sm text-stone-500 dark:text-slate-400">Listening attempts: {plays}. The transcript unlocks after two plays.</p>
      </div>
      {showScript && <div className="rounded-3xl p-7 bg-white dark:bg-slate-800 border border-stone-200 dark:border-slate-700 whitespace-pre-line leading-relaxed">{task.script}</div>}
      <QuestionSet questions={task.questions} section="listening" onComplete={onComplete} saved={saved}/>
    </div>
  );
}
...
function WritingTask({ task, onComplete, saved }) {
  ...
  const save = () => {
    ...
    onComplete('writing', words >= 80 && words <= 110 ? 1 : 0, 1, {}, []);
  };
  return (
    <div className="space-y-7"><div className="rounded-3xl p-7 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800"><h2 className="text-2xl font-extrabold font-serif mb-3">Short writing</h2><p className="leading-relaxed">{task.prompt}</p></div><div className="rounded-3xl p-6 bg-white dark:bg-slate-800 border border-stone-200 dark:border-slate-700"><textarea value={text} onChange={e => setText(e.target.value)} rows={10} placeholder="Write your paragraph here..." className="w-full p-4 rounded-2xl border border-stone-300 dark:border-slate-600 bg-stone-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500"/><div className={`mt-2 text-sm font-bold ${words >= 80 && words <= 110 ? 'text-emerald-600' : 'text-stone-500'}`}>{words} words · target: 80–110</div><div className="mt-5 grid sm:grid-cols-2 gap-2">{task.checklist.map(item => <div key={item} className="flex items-center gap-2 text-sm"><CheckCircle className="w-4 h-4 text-blue-500"/>{item}</div>)}</div><button onClick={save} className="mt-6 px-7 py-3 rounded-xl bg-slate-900 dark:bg-blue-600 text-white font-bold">Save writing</button>{saved && <span className="ml-4 text-sm font-bold text-emerald-600 dark:text-emerald-400">Saved in this browser</span>}</div></div>
  );
}

export default function Homework20260901({ onOpenHomework4, onOpenHomework3, onOpenHomework2, onOpenHomework1, darkMode, toggleDarkMode }) {
  ...
  useEffect(() => {
    fetch(DATA_URL).then(response => { if (!response.ok) throw new Error(`Homework data returned ${response.status}`); return response.json(); }).then(setData).catch(errorValue => setError(errorValue.message));
    try {
      const savedProgress = localStorage.getItem(PROGRESS_KEY);
      const savedMistakes = localStorage.getItem(MISTAKE_KEY);
  ...
  const complete = (section, score, total, answers, questions) => {
    const nextProgress = { ...progress, [section]: { score, total } };
    setProgress(nextProgress);
    try { localStorage.setItem(PROGRESS_KEY, JSON.stringify(nextProgress)); } catch (_) {}
    if (questions.length) {
  ...
  const tabs = [
    ['notes', 'Lesson Notes', BookOpen], ['grammar', 'Grammar', PenTool], ['flashcards', 'Flashcards', Sparkles],
    ['daily', 'Daily Use', RefreshCw], ['practice', 'Practice', PenTool], ['conversation', 'Conversation', MessageCircle],
    ['reading', 'Reading', BookOpen], ['listening', 'Listening', Headphones], ['writing', 'Writing', PenTool], ['quiz', 'Final Quiz', Star]
  ];
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 pb-20">
        ...
        <nav className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-10 gap-2 mb-10 p-2 rounded-2xl bg-white dark:bg-slate-800 border border-stone-200 dark:border-slate-700">{tabs.map(([id, label, Icon]) => <button key={id} onClick={() => { setTab(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`p-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 ${tab === id ? 'bg-slate-900 dark:bg-blue-600 text-white' : 'hover:bg-stone-50 dark:hover:bg-slate-700'}`}><Icon className="w-4 h-4"/>{label}{progress[id] && <CheckCircle className="w-3 h-3 text-emerald-400"/>}</button>)}</nav>
